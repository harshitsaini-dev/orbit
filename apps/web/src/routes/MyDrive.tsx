 ...
 *
 * Well below that cap, and the reason is a hard limit rather than a taste.
 * The API sits behind a proxy that closes a request after a hundred seconds,
 * and a batch is a run of provider calls held open by one request. Fifty, six
 * at a time, is a few seconds - far enough from that ceiling that a slow
 * provider, or a rate limit being waited out, still lands inside it.
 *
    ...
    >
      ...
      <section className="clay" style={{ padding: 'clamp(0.75rem, 2vw, 1.25rem)' }}>
        {/*
          ...
          * only exists inside a modal is a count nobody can see beside the
          * list it is emptying. `aria-live` is polite rather than assertive:
          * worth announcing, not worth interrupting with on every batch.
          */}
        {deleteProgress && (
